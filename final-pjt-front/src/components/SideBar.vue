  ...
  <div class="container" :class="{ show: showSidebar }">
    ...
    <div class="navigation-links">
      ...
      <transition-group name="fade" v-if="this.$store.state.username">
        <div @click="goToMy" v-show="showLink" key="1">
          <router-link :to="`/${userId}`"
            ><img
              :src="`http://127.0.0.1:8000${this.$store.state.userImg}`"
              alt="#"
          /></router-link>
        </div>
        <div v-show="showLink" key="2">
          <h4>{{ this.$store.state.nickname }}</h4>
        </div>
        <div v-show="showLink" key="3">
          <router-link :to="`/${userId}/main`">Home</router-link>
        </div>
        <div v-show="showLink" key="4">
          <router-link to="/movies">Movies</router-link>
        </div>
        <div @click="goToMy" v-show="showLink" key="5">
          <router-link :to="`/${userId}`">Profile</router-link>
        </div>
        ...
        <div v-show="showLink && this.$store.state.token" key="7">
      ...
      <transition-group v-else>
        <div v-show="showLink" key="1" class="alert-message">
          로그인이 필요한 서비스입니다.
        </div>
      </transition-group>
    </div>
...
@mixin nav-childs($values...) {
  @each $var in $values {
    &:nth-child(#{$var}) {
      transition: transform linear calc(0.05s * #{$var}), display 0.5s;
    }
  }
}
.fade-enter-active,
